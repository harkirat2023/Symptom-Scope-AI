...
async def search_hospitals(
    ...
    sort_by: str | None = Query(None, description="Sort field: rating, distance_km, bed_count"),
    sort_order: str = Query("desc", description="Sort direction: asc or desc"),
    limit: int = Query(20, ge=1, le=100, description="Max results"),
    _: str = Depends(get_current_user),
    hospital_service: HospitalService = Depends(),
):
    results = hospital_service.search(
        query=q,
        location=location,
        specialty=specialty,
    ...
    return HospitalSearchResponse(
        results=[HospitalResponse(**h) for h in results],
        total=len(results),
        locations=hospital_service.get_locations(),
        specialties=hospital_service.get_specialties(),
    )
...
async def get_hospital_locations(
    ...
    return hospital_service.get_locations()
