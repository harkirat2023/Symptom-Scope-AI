...
check_cmd mongod    || { err "mongod not found"; exit 1; }
check_cmd node      || { err "node not found"; exit 1; }
...
if pgrep -x mongod >/dev/null 2>&1; then
    ...
    for i in $(seq 1 30); do
        if command -v mongosh >/dev/null 2>&1; then
            ...
            python -c "import socket;s=socket.socket();s.settimeout(1);s.connect(('localhost',27017));s.close()" 2>/dev/null && break
        fi
        sleep 1
    done
...
if curl -sf http://localhost:8080/health >/dev/null 2>&1; then
    ...
    curl -s http://localhost:8080/health | python -m json.tool 2>/dev/null || true
else
